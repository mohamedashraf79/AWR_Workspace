...
int main(int argc, char** argv)
{
  ...
  private_node_handle.param<std::string>("port", port, "/dev/uno");
          ...
          while (input.length() >= 28) {
            ...
            if (data_packet_start != std::string::npos) {
              if ((input.length() >= data_packet_start + 28) && (input.compare(data_packet_start + 26, 2, "\r\n") == 0)) {
                ...
                int16_t raw_temp = (((0xff & (char)input[data_packet_start + 22]) << 8) | 0xff & (char)input[data_packet_start + 23]);
                double temperature = raw_temp / 340.0 + 36.53;
                ...
                temp_msg.header.stamp = measurement_time;
